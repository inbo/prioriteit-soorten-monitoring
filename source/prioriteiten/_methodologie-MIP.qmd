...
data_vb_scaled <- data_vb %>%
  mutate(
    criterium_1 = criterium_1 / max(data_vb$criterium_1),
...
model <- MIPModel() %>%
  ...
  set_objective(
    sum_expr(w[j] * s[i, j] * x[i], i = 1:nb_soort, j = 1:nb_criteria),
    sense = "max"
  ) %>%
...
model <- MIPModel() %>%
  ...
  add_constraint(
    sum_expr(x[i] * c[i], i = 1:nb_soort) +
      sum_expr(y[g] * cg[g], g = 1:nb_groep) <=
      110
  ) %>%
